...
import br.com.ume.application.features.transaction.handleTransactionFailed.useCase.HandleTransactionFailedUseCaseImpl
...
class HandleTransactionFailedUseCaseTests {
    ...
    private val handleTransactionFailedUseCase = HandleTransactionFailedUseCaseImpl(paymentGatewayMock, coordinatorServiceMock)

    private val defaultTransaction = TransactionDtoBuilder.build()
    private val defaultPayment = PaymentDtoBuilder.build()
    private val defaultReason = "Falha no pagamento PIX"
    ...
    fun `Should cancel contract`() {
        ...
        // When
        handleTransactionFailedUseCase.execute(defaultTransaction)
    ...
    fun `Should throw if find payment fails`() {
        ...
        // When
        assertThrows<Exception> { handleTransactionFailedUseCase.execute(defaultTransaction) }
